"""
Фаза 1 — детекция позы человека с веб-камеры на YOLO26 (TensorRT engine).
Модель обнаруживает ключевые точки (нос, глаза, плечи, локти, кисти, бёдра, колени, стопы).
Потоковая архитектура сохранена.

Запуск:  python detect_pose.py
Выход:   клавиша q
"""
import threading
import time
...
# --- настройки ---
MODEL = "yolo26x-pose.engine"   # модель позы (или .pt)
CAM_INDEX = 0
CONF = 0.35
DEVICE = 0
MIRROR = True
WIN = "YOLO26 - pose estimation (q to quit)"
...
class InferPoseThread(threading.Thread):
    ...
    def run(self):
        ...
        while self.alive:
            ...
            # plot() рисует скелет и ключевые точки
...
def main():
    model = YOLO(MODEL, task="pose")   # task="pose"
    ...
    infer = InferPoseThread(model, raw_box, out_box)
    capture.start()
    infer.start()
